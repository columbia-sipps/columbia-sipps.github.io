...
As an example, let's look into the Study1 dataframe  
...
#OR even these below will work:
#mydata <- read.csv("Study1.csv")
...
```{r}
mydata2 <- mydata %>% 
  dplyr::select(., ID, Condition, Age, starts_with("Day"), -ends_with("4"))
```
...
```{r}
mydata_cond1 <- mydata2 %>%
  filter(Condition == 1) # remember the relational operators? we are seeing them in action here!
```

You can also separately get those in condition 2 
```{r}
mydata_cond2 <- mydata2 %>%
  filter(Condition == 0) # remember the relational operators? we are seeing them in action here!
```
...
```{r}
is.na(mydata$Age)# returns TRUE if x is missing and returns FALSE is x is present
...
```{r, eval = FALSE}
# Write data to a .csv
write_csv(mydata2, "Study1_Clean.csv") # this is kind of the opposite to the read_csv function!
```
...
save(mydata2, mydata_cond1, mydata_cond2, file = "Study1_clean.rda") 
# the `file=` argument is very important here! 
