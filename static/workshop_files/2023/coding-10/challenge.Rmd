...
ggplot(data = rainfall, aes(x = city, y = inches_rain)) +
  geom_col() +
  labs(y = 'annual rainfall (in)', title = 'average precipitation for US cities')
```
...
ggplot(data = rainfall, aes(x = fct_reorder(city, inches_rain), y = inches_rain,
                            fill = inches_rain)) +
  geom_col() +
  geom_hline(yintercept = avg_rainfall, linetype = 'dashed') +
  annotate(geom = 'text', x = 1, y = avg_rainfall + 1,
           label = 'average rainfall across cities',
...
```{r}
ggplot(data = pokemon, aes(x = Type1, y = Speed, fill = Type1)) +
  geom_boxplot() +
  labs(x = 'primary type', y = 'speed',
       title = 'speed stats for pokemon of different types')
```
...
ggplot(data = pokemon, aes(x = Type1, y = Speed, fill = Type1)) +
  ...
  theme(legend.position = 'none') +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  facet_wrap(~ Generation, nrow = 2, ncol = 3)
```
...
ggplot(data = legendary, aes(x = Attack, y = Defense, color = Type1)) +
  geom_point(shape = 18, size = 5) +
  annotate(geom = 'text', label = max_attack$Name,
           x = max_attack$Attack - 2, y = max_attack$Defense,
           hjust = 'right') +
  annotate(geom = 'text', label = max_defense$Name,
           x = max_defense$Attack - 2, y = max_defense$Defense,
           hjust = 'right') +
  labs(title = 'attack & defense stats of legendary pokemon') +
  theme_bw()
```
