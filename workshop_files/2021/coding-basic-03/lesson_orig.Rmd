...
Welcome to the Programming Fundamentals workshop - the 3rd workshop in the Basic coding track! 
...
Let's say we are interested in the mean number of words written by Shakespeare in his plays. So, we want to find the mean of the n.words column. Here are the steps we'd have to take to do this:
...
#Second argument sounds for how many rows you want; 10 is for 10 rows of data
...
```{r}
# this is commented out so that both types of statements don't run
# rename(mydata, Neuroticism = Personality)
```
...
```{r}
# is.na(x) returns TRUE where x is missing and returns FALSE where x is present
...
### Other ways to address missing data:

#### Recoding Values to Missing. 

If you are getting a dataset that was entered in SPSS, chances are good that missing values will be coded as -99 or 999 or another implausible value. Therefore, to conduct analyses in R, you'd need to change these values to NA.
```{r}
# recode 99 to missing for variable Age
...
Arithmetic functions on missing values yield missing values, unless you include na.rm = TRUE
```{r}

```

Advanced Handling of Missing Data: Most modeling functions in R offer options for dealing with missing values. You can go beyond pairwise of listwise deletion of missing values through methods such as multiple imputation. Good implementations that can be accessed through R include Amelia II, Mice, and mitools.
