...
Let's plot what our effect coded interacrtion looks like. We need to first change the variables into vectors for the following code to work.
...
dfhigh <- data.frame(
  ...
  type.e = .5) # You put whatever your categorical variable is here, and set it equal to one of the values, 
...
plot <- ggplot(high, aes(prestige.c, fit)) +
  geom_point(data = df, # use original data to plot datapoints 
             aes(prestige.c, income, color = as.factor(type.e)),# note, you must use as.factor here
             # to turn this into a variable with two levels, or else it will try to plot this as continuous
             # and the code will not run, since it is not a continuous variable
             alpha = .4, size = 1, show.legend = T) +
            scale_color_manual(values = c("-.5" = "green", ".5" = "orange")) + 
  labs(x = "Prestige",
       y = "Income",  title = " ") +
  ...
  geom_ribbon(data = low, aes(ymin=lwr, ymax=upr), 
              alpha = .3, fill = "green") +
  geom_line(data = low, aes(prestige.c, fit), 
            size = 2, color = "green") 

plot + scale_color_manual(name = "Job Type", labels=c("BC", "WC"), values = c("green", "orange"))
```
