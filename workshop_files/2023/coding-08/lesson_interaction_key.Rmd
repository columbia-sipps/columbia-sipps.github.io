...
>- The prestige slope tells us that with each additional unit of prestige (when averaging across both job types) we expect income to increase by ~119.
>- The type slope tells us that -- when holding prestige at the average (mean) level -- as we move from blue collar to white collar jobs we can expect income to decrease by ~1034.
>- Note that our language for interpreting the slope of prestige and the slope of type has changed a little bit. Since we have an interaction in our model, these main effects now get interpreted as conditional effects. That is, when there is an interaction in the model, you interpret the slope of one variable as the effect of X1 on Y when holding X2 at 0. This is why centering matters! In this example, when prestige is at 0, that means the average or mean level of prestige. And, when job type is at 0, that means when averaging across job type. 
...
>- The intercept tells us that for **blue collar jobs** of **average prestige**, we can expect an income of ~5745. Note that this value is different than the effect coded version because of how we centered the job type variable!
>- The prestige slope tells us that with each additional unit of prestige **in blue collar jobs**, we expect income to increase by ~161. Note that this is ALSO different than the effect coded version because of how we centered the job type variable!
>- The type slope tells us that as we move from blue collar to white collar jobs we can expect income to decrease by ~1034. Note that this is the SAME as the effect coded version because in both m2 and m3, the prestige variable was centered in the same way!
>- The interaction term tells us that as we move from blue collar to white collar jobs, the slope of prestige on income (i.e., the degree to which income increases with prestige) is ~84 units **lower**. In other words, prestige **has a weaker effect** on income for white collar vs. blue collar jobs. Note that this is ALSO the same as the effect coded version!

To summarize, the main effect of `prestige.c` is different in the effect coded vs. dummy coded versions of the models! This is because in the dummy coded version, a value of 0 for job type refers to blue collar jobs, NOT the average across both job types. So, whereas the prestige slope in the first model showed us the effect of prestige on income when averaging across job type, this version of the slope is **only** for blue collar jobs.

We'll go over how to visualize all of these effects in another lesson, but here's some pre-written code to do so now. Visualization is **really helpful** for interpreting interactions — especially when you get into more complex cases like 3-way interactions or when your interactions might not be so clear cut.
...
First, let's mean center the column `women`which refers to the percentage of women in a particular job. (Remember: all continuous variables should be mean centered.)

```{r}
df$women.c <- df$women - mean(df$women)
```

Next, let's run a model `m4` predicting `income` from mean centered `prestige` and `women,`as well as the interaction between these two IVs. 
...
It's also important to note that all of these effects are significant (as denoted by the *** or the p-value). The concept of p-values is beyond the scope of today's topic, but you can basically interpret these effects to be **statistically significant**.
...
Now, let's try one more continuous by continuous interaction. Here let's see if the average education level of those in an occupation interacts with the prestige of an occupation in predicting income.
...
Again, note that the intercept, and the slopes for prestige and education and statistically significant with p < 0.05. The interaction between education and prestige is not statistically significant (p = 0.065), but is trending in the direction of reaching statistical significance. 
...
>- The supp slope tells us that with **when going from OJ to VC type of supplement** at **1.0 mg/day dosage*, we expect tooth length growth to decrease by ~5.9. So, a 1.0mg/day dose of OJ has a bigger effect on tooth growth than a 1.0mg/day VC dose.
...
>- The supp slope tells us that with **a change from OJ to VC in type of supplement**, averaged across both dosages, we expect tooth length growth to decrease by ~2.9. In other words, across both dosages, OJ is associated with greater increases in tooth length than VC.  
>- The dose slope tells us that as we move from **1.0 mg/day to 2.0 mg/day dosage**, averaging **across both supplement types**, we can expect tooth length growth to increase by ~6.4.
>- The interaction term tells us that as we move from **1 to 2 mg/day of dosage**, the slope of *supplement type* on tooth length is ~6 units **higher** for **VC vs. OJ**. In other words, increasing the dosage from 1.0 to 2.0 mg/day **has a stronger effect** on tooth length when using VC vs. OJ.
