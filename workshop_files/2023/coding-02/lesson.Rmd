...
Welcome to the Programming Fundamentals workshop! 
...
```{r}
 # TRUE - Like multiplying by 1. the other number stays the same!
```


```{r}
 # FALSE -  Like multiplying by 0. the other number becomes 0!
```
...
```{r}
# create a vector of names of fruits called char.vector
...
```{r}
#create a na.vector with some elements, one of which is NA
...
```{r}
 # Here every element of na.vector gets 1 added to it, except the NA
```

When you call summarizing (many-to-one) functions on a vector containing `NA`, the summary value will be `NA`...

```{r}
 # here, your answer is NA, because R can find the mean/average of some numbers when one of them is NA
```

unless you specify in the function that you want the value to be calculated as if the `NA`s aren't there.

```{r}
# calculate mean of na.vector excluding the na
...
[Here](https://swcarpentry.github.io/r-novice-gapminder/02-project-intro/){target="_blank"} are some more details about best practices to store your data files and how to use projects. This may be a little advanced for now, but be sure to visit this when you're working on your project this summer.
