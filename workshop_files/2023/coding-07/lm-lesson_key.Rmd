...
subtitle: "Introduction to Linear Models - Answer Key"
output:
  html_document:
    toc: true
    toc_float: true
...
df_clean$type.d <- dplyr::recode(df_clean$type, "bc" = 0, "wc" = 1)
...
df_clean$type.e <- dplyr::recode(df_clean$type, "bc" = -0.5, "wc" = 0.5)
...
m1 <- lm(income ~ prestige.c, data = df_clean)
...
# fit model
m2 <- lm(income ~ type.d, data = df_clean)
summary(m2)
...
# fit model
m3 <- lm(income ~ type.e, data = df_clean)
...
In other words: Our intercept tells us that **for the average job (across both blue & white collar)**, we predict that the income will be ~5213. Our type.d slope tells us that as we move from a blue collar job to a white collar job, we can expect income to decrease by ~322. (Though again, this effect isn't significant.)
...
m4 <- lm(income ~ prestige.c + type.e, data = df_clean)
summary(m4)
