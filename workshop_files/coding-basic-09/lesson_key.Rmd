...
This tutorial does not demonstrate *every* possible ggplot aesthetic modification (and you really can adjust nearly every feature of a plot with `ggplot2`!). The tutorial walks through common aesthetic changes I find myself using in my own plots, that I hope will apply to your plotting needs as well.
...
ggplot(iris, aes(x = Petal.Length, y = Petal.Width,
                 color = Species)) +
  geom_jitter(size = 4, shape = 'triangle')
```
...
ggplot(us_states,
       aes(x = fct_reorder(state, area), 
           y = area)) + 
  geom_col(color = "forestgreen", fill = "#825324") +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  labs(x = "US state",
       y = "area (sq mi)")
