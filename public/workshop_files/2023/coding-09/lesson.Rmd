...
At this point, you know how to run and interpret various kinds of linear models using R. However,correctly interpreting the results of models like these can be tricky and takes some practice. Learning how to visualize your model results is one of the best ways to understand the effects of your models. During this lesson, we'll review how to run a few new models, and walk you through the steps of correctly plotting them.
...
This week, we're going to use another built-in dataset from the `psych` package, called `affect` (run `?affect` in your console to get a brief overview of what this consists of). This dataset contains results of two studies that investigated the relationship between personality traits and emotional responses to various types of movies.

For these exercises, let's consider only two of the movie conditions included in this study: one in which participants viewed a documentary about the liberation of the Bergen-Belsen concentration camp (`Film = 1`), and one in which they viewed the comedy Parenthood (`Film = 4`).
...
# center traitanx (two ways)
df$traitanx.c <- df$traitanx - mean(df$traitanx)

df <- df %>% mutate(traitanx.c = traitanx - mean(traitanx))
...
Next, we will learn how to plot regression lines using `ggplot` and the `predict` function, which provides more flexibility when creating your visualizations.
...
- Our interaction term tells us that the degree to which negative affect increases as a function of trait anxiety (i.e., the slope of anxiety on affect) is 0.026 units greater when participants watched a comedy versus a documentary. In other words, the relationship between trait anxiety and negative affect was slightly stronger when participants watched a comedy. However, note here that **the interaction between these variables is NOT significant**. This allows us to conclude that the relationship between anxiety and negative affect does NOT significantly depend on the type of movie participants saw.
